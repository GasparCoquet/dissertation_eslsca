...
def inline(text):
    """-> list of (text, bold, italic). Handles **bold** and *italic*."""
    out, idx = [], 0
    for m in re.finditer(r'\*\*(.+?)\*\*|\*(.+?)\*', text):
        if m.start() > idx:
            out.append((text[idx:m.start()], False, False))
        ...
        idx = m.end()
    if idx < len(text):
        out.append((text[idx:], False, False))
...
def build_docx(blocks):
    ...
    center("MBA — Trading & Financial Markets", 12)
...
def build_pdf(blocks):
    ...
    def front(pdf):
        ...
        pdf.set_font(FONT, "", 12); pdf.multi_cell(0, 7, "MBA - Trading & Financial Markets", align="C"); pdf.ln(8)
